...
- 内置版式护栏：目录使用真实页码，图表解读与来源安全换行，长表格自动改为卡片/拆页/横向方案，章节导入页按阅读节奏使用。
- 中文字体优先使用本机 `STKaitiSC-Regular/Bold/Black`；不内置任何字体文件。

## 输出标准

最终 PDF 应面向客户阅读，而不是展示内部分析过程。正文应使用白底段落、蓝色小标题、细分隔线和自然叙事；避免浅灰白板框、三栏推理框和大面积空白。

最终 PDF 不应出现以下内部标签：`证据：`、`含义：`、`逻辑链条`、`事实基础`、`对客户启示`、`本章逻辑`、`竞品`。可使用更客户化的表达，例如“市场表现”“战略打法”“行业启示”“章节导读”“重点企业”“主要参与者”“竞争参与者”。

交付前应渲染抽查目录页、图表页、企业/表格页和策略建议页，确认没有右侧裁切、标签重叠、标题孤字换行或页脚遮挡。
